...
export function fitColliderToModel(body, modelHolder) {
  ...
  // Wall clamp uses AABB visual extent so a bad disc read cannot collapse the
  // playable radius and fling beys through the stadium.
...
export function applyOrbitDrift(body, spin) {
  ...
  // Perpendicular to velocity; spinSign keeps the curve direction tied to the
  // bey's own spin so it always arcs the same consistent way.
